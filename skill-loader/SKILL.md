...
在执行项目级安装或创建 CLI 专属目录前，必须先确定目标 CLI。

判断规则：

1. 如果用户明确指定目标 CLI（如“为 claude 安装”“为 trae 安装”），优先使用用户指定值。
2. 如果当前会话、执行入口或上下文能明确表明运行在 Claude Code，则目标 CLI 为 `claude`。
3. 如果当前会话、执行入口或上下文能明确表明运行在 Trae，则目标 CLI 为 `trae`。
4. 如果存在冲突信号、没有可靠信号，或无法判断当前运行环境，则不要猜测，也不要创建任何 CLI 专属目录，应先询问用户。

询问示例：

> 无法确定当前 CLI 运行环境。请选择要为哪个 CLI 安装 skill：`claude`、`trae`，或提供其他目标 CLI。

用户选择后，只能为被选择的目标 CLI 创建对应目录。若用户选择其他 CLI，应继续确认目标目录策略后再执行。
...
- 执行安装前必须先确定目标 CLI
- 创建 CLI 专属目录时，只能创建当前目标 CLI 对应的一个目录，例如目标为 `claude` 时只创建 `claude`，目标为 `trae` 时只创建 `trae`
- 不允许为了兼容性预创建 `claude`、`trae` 等多个候选 CLI 文件夹
- 如果目标 CLI 无法确定，必须询问用户；用户选择前不得创建任何 CLI 专属目录
- 如果项目中已存在其他 CLI 目录，本次安装不得额外创建或删除无关目录，除非用户明确要求
- `behavior.confirmBeforeInstall` 只用于额外控制安装前确认；但它不应覆盖“明确点名直接安装”的主规则
- 安装成功后明确告知已装到当前项目，并说明使用的目标 CLI
...
- 默认项目级安装，不污染全局
- 安装前确定目标 CLI，只创建当前环境需要的 CLI 专属目录
- 无法确定当前运行环境时先询问用户，不猜测、不预创建多个 CLI 文件夹
- 配置驱动，不硬编码用户环境
